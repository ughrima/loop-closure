...
if __name__ == "__main__":
    ...
    # Load icp loop closures
    try:
        icp_closures = np.load("/home/agrima/loop-closure-lpgw/icp/icp_detected_loop_closures.npy", allow_pickle=True)
        print(f"Loaded {len(icp_closures)} icp loop closures.")
    except FileNotFoundError:
        print("Error: icp_detected_loop_closures.npy not found.")
        exit(1)
    except Exception as e:
        print("Error loading icp loop closures:", e)
        exit(1)

    # Filter closures to ensure indices are valid
    valid_closures = [
        (scan_i, scan_j) for scan_i, scan_j in icp_closures
