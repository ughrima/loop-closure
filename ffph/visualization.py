...
def visualize_and_save_loop_closure(scan_i, scan_j, pcd_files, output_folder):
    ...
    cloud_i.paint_uniform_color([0,1,0])  # Red for scan i
    cloud_j.paint_uniform_color([1, 0,0,])  # Blue for scan j
    ...
    output_path = os.path.join(output_folder, f"loop_closure_{scan_i}_{scan_j}.png")
...
if __name__ == "__main__":
    ...
    # Load ffph loop closures
    try:
        ffph_closures = np.load("/home/agrima/loop-closure-lpgw/ffph/fpfh_detected_loop_closures.npy", allow_pickle=True)
        print(f"Loaded {len(ffph_closures)} ffph loop closures.")
    except FileNotFoundError:
        print("Error: ffph_detected_loop_closures.npy not found.")
        exit(1)
    except Exception as e:
        print("Error loading ffph loop closures:", e)
        exit(1)

    # Filter closures to ensure indices are valid
    valid_closures = [
        (scan_i, scan_j) for scan_i, scan_j in ffph_closures
        if scan_i < num_pcd_files and scan_j < num_pcd_files
    ]
    print(f"Valid loop closures for visualization: {len(valid_closures)}")
    ...
    print(f"Randomly selected {len(sampled_closures)} loop closures for visualization.")

    # Visualize and save the sampled closures
    for i, (scan_i, scan_j) in enumerate(sampled_closures):
        print(f"Visualizing and saving loop closure {i+1}: Scan {scan_i} and Scan {scan_j}")
